...
class MultiHeadAttention(Layer):
    def __init__(self, d_model, num_heads):
        ...
        self.d_model = d_model
        self.num_heads = num_heads
        self.dimension_per_head = self.d_model // 2
        self.wq = Dense(d_model, activation=None)
        self.wk = Dense(d_model, activation=None)
        self.wv = Dense(d_model, activation=None)
    ...
    def call(self, q, k, v, mask):
        ...
        # outputs = self.depthwise1(outputs)
        ...
        scaled_attention_1 = tf.expand_dims(scaled_attention_1_st, axis=-1)
        ...
        scaled_attention_2 = tf.expand_dims(scaled_attention_2_st, axis=-1)
        ...
        outputs = tf.concat(par_output, axis=-1)
        '''
        scaled_attention_1_st = scaled_attention_1_st + scaled_attention_2
        scaled_attention_2_st = scaled_attention_2_st + scaled_attention_1
        par_output = [scaled_attention_1_st, scaled_attention_2_st]
        outputs = tf.concat(par_output, axis=-1)
        return outputs
